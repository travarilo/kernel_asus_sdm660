...
struct sx93XX {
	/* either i2c_client or spi_client */
	void *bus;
	/* common device struction for linux */
	struct device *pdev;
	/* device specific struct pointer */
	void *pDevice;
	...
	/* get whether nirq is low (platform data) */
	...
	struct mutex mutex;
#else
	/* Spin Lock used for nirq worker function */
	spinlock_t lock;
#endif
	/* irq number used */
	int irq;
	 ...
	 */
	char irq_disabled;
	/* older models need irq timer for pen up cases */
	u8 useIrqTimer;

...
int ov8856_read_mask;
uint16_t ov8856_mask;
...
int msm_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (strcmp(sensor_name, "ov8856_chicony_front") == 0 &&
	    id_match == 1 && ov8856_read_mask == 0) {
		...
		pr_err("ov8856 mask read from 0x700f is 0x%x,0x02 is 1b, other is 1a",
		       ov8856_mask);
	...
	if (strcmp(sensor_name, "gc5025_ofilm_13m") == 0 &&
	    gpio_get_value(SUB_CAM_ID_PIN)) {
		pr_err("Sub camera is 16+5M ofilm Configuration");
		return -ENODEV;
	}

	if (strcmp(sensor_name, "hi556_holitech_13m") == 0 &&
	    gpio_get_value(SUB_CAM_ID_PIN)) {

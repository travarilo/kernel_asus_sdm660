...
{	char write_data[32] = {0};
	...
	if (write_data[0] == '1') {
		...
		pr_info("%s: write enable 1 soc = %d, limit-value= %d!\n",
		...
		if (online)
			power_supply_changed(smbchg_dev->batt_psy);
		pr_info("%s: write enable 0, no limit ,charging!\n", __func__);
	}
	pr_info("%s: charger_limit_enable_flag = %d\n",
					...
					size_t size, loff_t *ppos)
{
	...
	pr_info("%s: limit-value= %d, limit-flag= %d, current-soc = %d\n",
		__func__, charger_limit_value, charger_limit_enable_flag, soc);
...
static int init_proc_charger_limit(void)
{
	...
	limit_enable_entry = proc_create(CHARGER_LIMIT_EN_PROC_FILE, 0666,
					 NULL,
					 &charger_limit_enable_proc_ops);

	if (limit_enable_entry == NULL) {
		pr_err("%s: create_proc entry %s failed\n",
		       __func__, CHARGER_LIMIT_EN_PROC_FILE);
		return -ENOMEM;
	} else {
	...
	if (limit_entry == NULL) {
		pr_err("%s: create_proc entry %s failed\n",
		       __func__, CHARGER_LIMIT_PROC_FILE);
		return -ENOMEM;
	} else {
		pr_info("%s: create proc entry %s success",
			__func__, CHARGER_LIMIT_PROC_FILE);
		...
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (tmp == 0) {
		...
		pr_info("%s: demo_app_property_flag = 0\n", __func__);
	} else if (tmp == 1) {
		demo_app_property_flag = true;
		pr_info("%s: demo_app_property_flag = 1\n", __func__);
	}
	return len;
}
...
static DEVICE_ATTR(demo_app_property, 0664, demo_app_property_show,
...
int32_t get_ID_vadc_voltage(void)
{
	...
	pr_info("%s: adc=%d adc_result.physical=%lld adc_result.chan=0x%x\n",
...
static int smb2_probe(struct platform_device *pdev)
{
	...
	pr_info("%s: ADC_SW_EN=%d,ADCPWREN_PMI_GP1=%d\n",
		__func__, gpio_ctrl->ADC_SW_EN, gpio_ctrl->ADCPWREN_PMI_GP1);
	if (!gpio_ctrl)
		return -ENOMEM;
	...
	gpio_ctrl->ADC_SW_EN = of_get_named_gpio
		(pdev->dev.of_node, "ADC_SW_EN-gpios59", 0);

	rc = gpio_request(gpio_ctrl->ADC_SW_EN, "ADC_SW_EN-gpios59");
	if (rc)
		pr_err("%s: failed to request ADC_SW_EN-gpios59\n", __func__);
	else
		pr_info("%s: Success to request ADC_SW_EN-gpios59 %d\n",
			__func__, (int)gpio_ctrl->ADC_SW_EN);

	gpio_ctrl->ADCPWREN_PMI_GP1 = of_get_named_gpio
		(pdev->dev.of_node, "ADCPWREN_PMI_GP1-gpios34", 0);
	...
	if (rc)
		...
		pr_info("%s: Success to request ADCPWREN_PMI_GP1-gpios34 %d\n",
	...
	if (rc < 0) {
		pr_err("%s: create node demo_app_property failed! rc=%d\n",
		       __func__, rc);
		goto cleanup;
	}
	...
	pr_info("%s: enter1 HVDVP_reg=0x%x\n", __func__, HVDVP_reg);
	...
	pr_info("%s: enter2 HVDVP_reg=0x%x\n", __func__, HVDVP_reg);
	rc = smblib_read(smbchg_dev, USBIN_AICL_OPTIONS_CFG_REG,
			 &USBIN_AICL_reg);

	pr_info("%s: enter1 USBIN_AICL_reg=0x%x\n", __func__, USBIN_AICL_reg);
	rc = smblib_masked_write(smbchg_dev,
	...
	if (rc < 0)
		pr_err("%s: Failed to set USBIN_OPTIONS_1_CFG_REG\n", __func__);
	pr_info("%s: enter2 USBIN_AICL_reg=0x%x\n", __func__, USBIN_AICL_reg);

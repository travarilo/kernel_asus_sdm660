...
static int stk_tune1_ct_det(struct stk3x1x_data *ps_data, uint32_t ps)
{
	...
	if (ct >= 0) {
		if (ct < STK_PS_CT_LIMIT) {
			...
			pr_info("%s: ct>STK_PS_CT_LIMIT,keep HT/LT\n", __func__);
			return 2;
		}
	}
	return 0;
				   ...
				   uint32_t ps_reading)
{
	...
	pr_info("%s:handle_state=%d,ps=%d,als=%d,nf_flag=%d,ps_distance_last=%d\n",
...
static int stk3x1x_suspend(struct device *dev)
{
	...
	if (ps_data->ps_enabled) {
		...
		if (device_may_wakeup(&client->dev)) {
			...
			if (err)
				pr_err("%s: set_irq_wake(%d) failed, err=(%d)\n",
...
static int stk3x1x_resume(struct device *dev)
{
	...
	if (ps_data->ps_enabled) {
		...
		if (device_may_wakeup(&client->dev)) {
			...
			if (err)
				pr_err("%s: disable_irq_wake(%d) failed, err=(%d)\n",
					__func__, ps_data->irq, err);
		}
#endif
	}

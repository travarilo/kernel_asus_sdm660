...
int create_gesture_node(void) {
	int error = 0;
        pr_info("[Nvt-ts] : Gesture Node initialized successfully \n");
        ...
        if (error) {
                pr_info("[Nvt-ts] : failed to create the gesture_node file in /sys/kernel/touchpanel \n");
        }
...
int32_t CTP_I2C_READ_DUMMY(struct i2c_client *client, uint16_t address)
{
	...
	if (ret < 0)
		pr_err("CTP_I2C_READ_DUMMY failed.(%d)\n", ret);
...
void nvt_ts_wakeup_gesture_report(uint8_t gesture_id)
{
	...
	if (keycode > 0 ) {
		if (is_double_tap == 1) {
			...
			pr_info("[NVT-ts] : gesture key code = %d\n", keycode);
